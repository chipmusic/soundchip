...
fn envelope_tests() {
    ...
    fn test_envelope(env: Envelope<f32>, start_time: f32, end_time: f32, delta: f32) {
        ...
        while time <= end_time {
            ...
            let b = if b < a {
                // Fixes incorrect interpolation wrapping around. Argh.
            ...
            let goal = lerp(env.knots[a].value, env.knots[b].value, local_time - a_time);
    ...
    test_envelope(
        envelope_with_times_starting_at_zero.clone(),
        0.0,
        35.0,
        0.125,
    );
    ...
    test_envelope(
        envelope_with_times_starting_at_zero.set_loop(LoopKind::Repeat),
        0.0,
        15.0,
        0.25,
    );
...
fn envelope_sort() {
    ...
    // Sorts on creation
    let mut env: Envelope<f32> = Envelope::from(knots.as_slice());

    let mut knot_time = -1.0;
    for knot in &env.knots {
        assert!(knot_time < knot.time);
        knot_time = knot.time;
    }

    // Additional sort
    env.knots.push(Knot::new(2.5, 1.0));
    env.knots.push(Knot::new(2.75, 1.0));
    env.sort_by_time();
